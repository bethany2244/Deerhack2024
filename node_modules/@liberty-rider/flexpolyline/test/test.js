...
const originalLines = readFileSync('../test/original.txt', { encoding: 'utf-8' }).split('\n');
const encodedLines = readFileSync('../test/encoded.txt', { encoding: 'utf-8' }).split('\n');
const decodedLines = readFileSync('../test/decoded.txt', { encoding: 'utf-8' }).split('\n');
...
function runTests() {
    originalLines.forEach((original, index) => {
        ...
        strictEqual(encodedInput, encoded);

        const expectedDecoded = parseLine(decoded);
        const resDecoded = decode(encodedInput);
        strictEqual(resDecoded.precision, expectedDecoded.precision);
        strictEqual(resDecoded.thirdDim, expectedDecoded.thirdDim || 0);
        strictEqual(resDecoded.thirdDimPrecision, expectedDecoded.thirdDimPrecision);
