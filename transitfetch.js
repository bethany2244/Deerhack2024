      ...
      .then((data) => {
        ...
        for (const path of data['routes'][0]['sections']) {
          ...
          if (path['type'] == 'pedestrian') {
            ...
            console.log(data);
            transitEndpoints.push([path['departure']['place']['location']['lat'], path['departure']['place']['location']['lng']]);
            transitEndpoints.push([path['arrival']['place']['location']['lat'], path['arrival']['place']['location']['lng']]);
            ...
            pathMarker = pathMarker.concat(new Date (path['arrival']['time']).getHours().toString());
            pathMarker = pathMarker.concat(':');
            pathMarker = pathMarker.concat(new Date (path['arrival']['time']).getMinutes().toString());
          }
